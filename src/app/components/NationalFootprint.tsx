...
import { MapPin, Navigation, ZoomIn, ZoomOut, Compass, Search, Info, Target, Zap, Building2 } from 'lucide-react'
...
const NationalFootprint = () => {
  ...
  useEffect(() => {
    const preventScroll = (e: WheelEvent) => {
      if (zoom > 1 && mapRef.current?.contains(e.target as Node)) {
        e.preventDefault()
      }
    }
  ...
  const filteredLocations = locations.filter(location =>
    ...
    location.capacity.toLowerCase().includes(searchQuery.toLowerCase())
  )

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Operational': return 'tag-success'
      case 'Under Construction': return 'tag-warning'
      case 'Pipeline': return 'tag-primary'
  ...
  const getMarkerColor = (status: string) => {
    switch (status) {
      case 'Operational': return 'bg-solar-success hover:bg-solar-success/80'
      case 'Under Construction': return 'bg-solar-warning hover:bg-solar-warning/80'
      case 'Pipeline': return 'bg-solar-accent hover:bg-solar-accent/80'
      default: return 'bg-solar-secondary hover:bg-solar-secondary/80'
    }
  }

  const handleMarkerClick = (slug: string, name: string) => {
    setActiveLocation(name)
    router.push(`/pages/projects/${slug}`)
  }

  const handleLocationClick = (slug: string, name: string) => {
    setActiveLocation(name)
    router.push(`/pages/projects/${slug}`)
  }
  ...
  const operationalCapacity = locations
    .filter(loc => loc.status === 'Operational')
    .reduce((sum, loc) => sum + (parseInt(loc.capacity) || 0), 0)

  const underConstructionCapacity = locations
    .filter(loc => loc.status === 'Under Construction')
    .reduce((sum, loc) => sum + (parseInt(loc.capacity) || 0), 0)

  const pipelineCapacity = locations
    .filter(loc => loc.status === 'Pipeline')
    .reduce((sum, loc) => sum + (parseInt(loc.capacity) || 0), 0)
            ...
            <div className="card card-glass card-interactive p-6 relative overflow-hidden">
              ...
              >
                ...
                <div className="w-full h-full max-w-4xl flex items-center justify-center relative">
                  <div className="text-center relative">
                    ...
                    <div className="relative mx-auto p-4 m-4">
                      {/* Bangladesh SVG Map */}
                      <svg
                      ...
                      >
                        ...
                        {/* Chittagong Division */}
                        <path
                          d="m 270.13233,436.57628 -0.31,3.81 -0.63,0.57 -0.38,-0.4 0.78,-2.27 -0.75,-0.56 -0.26,-0.81 1.22,-2.41 0.33,2.07 z m 2.38,8.41 0.88,-0.57 1.17,-3.22 -0.62,-2.27 -2.08,5.15 0.65,0.91 z m 5.74,-7.85 0.88,-1.58 0.03,-1.47 -1.17,-1.82 -1.9,1.09 -0.05,2.57 1.12,1.26 1.09,-0.05 z m 2.32,8.62 -1.72,-6.49 -1.38,0.42 -0.03,6.27 -7.52,19.23 1.22,1.31 1.17,1 0.83,-0.33 0.6,-2.85 4.14,-6.40 1.85,-4.56 0.84,-7.60 z m 5.08,-6.74 -1.20,-1.93 -2.29,2.02 1.33,1.76 2,-1.06 0.16,-0.79 z m -1.90,5.34 -0.57,-1.51 -1.17,1.09 0.78,2.85 0.96,-2.43 z m -4.07,-15.50 -0.31,0.98 1.32,7.28 1.25,1.01 1.64,-1.68 0.67,-2.36 0.64,-3.97 -2.60,-1.73 -2.61,0.47 z m 1.49,13.07 0.68,1.71 1.28,-1.29 -1.43,-1.54 -0.53,1.12 z m -27.32,-61.27 -0.41,-0.68 -2.46,-3
18,-1.35 2.07,0.06 0.29,-0.84 -1.34,-4.78 -1.43,-0.08 0.81,1.34 -1.63,-0.63 -1.64,2.25 -0.17,-1.51 1.02,-0.53 -2.49,-0.68 -1.02,0.78 -0.64,-0.32 2,-2.43 0.76,1.43 0.68,-0.62 2.49,-2.98 1.89,-4.58 -3.36,-1.78 0.64,-0.91 -0.30,-1.24 -2.16,-0.86 -0.23,-1.48 1.78,0.90 -0.01,-0.86 0.89,0.02 0.19,-1.69 1.42,-0.02 0.81,-0.89 -1.01,-1.50 1.78,-0.31 0.06,-1.57 -1.85,-0.81 0.84,-0.99 -0.46,-1.61 -1.53,1.55 -2.47,-1.57 -1.70,0.86 -0.35,-0.74 -1.62,-0.02 1.03,-0.77 -0.41,-1.36 0.69,0.36 0.38,-0.74 -0.68,0.05 0.02,-0.73 -1.20,-0.14 -0.18,-0.60 0.92,-0.61 1.37,0.29 -0.12,-0.93 1.30,-0.93 -1.39,-0.84 1.40,-2.27 -0.17,-1.88 -1.54,-0.91 -1.01,1.40 0.30,0.81 -1.33,1.05 1.25,1.50 -1.49,0.22 -1.51,-2.35 0.59,-2 0.83,-0.64 -0.41,-0.81 -0.97,-0.04 -1.16,-3.82 -2.12,-1.64 -3.47,-0.54 -1.25,2.26 -0.90,-0.22 -0.44,-1.07 -1.30,0.39 -1.08,-2.19 -3.37,-0.08 -0.10,-2.55 -0.64,0.05 1.96,-0.87 0.37,-1.25 -1.05,-1.44 -1.57,-0.39 0.33,-3 -1.24,-1.68 0.52,-0.86 2.35,-0.99 -2.93,-0.86 -0.20,-3.37 1.39,-0.33 1.18,1.06 1.50,-0.48 -0.26,-1.86 -1.96,-1.81 0.59,-1.13 -0.25,-2.88 1.64,-0.72 -0.52,-1.45 0,0 10.68,-2.95 4.64,0.43 6.70,-0.66 4.50,0.53 2.08,-1 z"
                          fill="#ecfdf5"
                          stroke="#10b981"
                          strokeWidth="1"
                        />
                    ...
                    {locations.map((location, index) => {
                      ...
                      const isVisible = !searchQuery || filteredLocations.some(loc => loc.name === location.name)
                      
                      if (!isVisible) return null

                      const positionStyle = location.position || { top: "50%", left: "50%" }
                      ...
                      return (
                        <motion.button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            handleMarkerClick(location.slug, location.name)
                ...
                <motion.div
                  ...
                  className="absolute bottom-0 left-0 card card-glass p-2 shadow-lg border border-solar-primary/20"
                >
                  <h4 className="text-sm font-semibold text-primary flex items-center gap-2 mb-3">
                    <Navigation className="h-4 w-4 text-solar-accent" />
                    Project Status
                  </h4>
                  <div className="space-y-2 text-xs">
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 bg-solar-success rounded-full"></div>
                      <span className="text-tertiary">Operational</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 bg-solar-warning rounded-full"></div>
                      <span className="text-tertiary">Under Construction</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 bg-solar-accent rounded-full"></div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-solar-accent text-sm font-semibold mb-2">
                        Explore the Map
                      </p>
                      ...
                      <p className="text-tertiary text-xs">
                        • {isTouchDevice ? 'Pinch to zoom and drag to move' : 'Drag to move when zoomed in'}
                      </p>
                    </div>
                    <button
                      onClick={() => setShowInstructions(false)}
                      className="text-tertiary hover:text-primary text-xs flex-shrink-0"
                    >
                      ✕
                    </button>
                  </div>
              ...
              >
                {zoom > 1 ? (
                  ...
                  <span className="text-solar-accent text-sm font-medium">💡 Click any marker to explore projects</span>
                )}
              </motion.div>
            </div>
          ...
          >
            ...
            >
              <div className="relative">
                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-tertiary h-4 w-4" />
                <input
                  type="text"
                  placeholder="Search projects by name, capacity, or status..."
                  value={searchQuery}
                  onChange={handleSearch}
                  className="input w-full pl-12 pr-10 text-base"
                />
                {searchQuery && (
                  <button
                    onClick={clearSearch}
                    className="absolute right-4 top-1/2 transform -translate-y-1/2 text-tertiary hover:text-solar-accent transition-colors"
                  >
                    ✕
                  </button>
                )}
            ...
            >
              {(searchQuery ? filteredLocations : locations).map((location, index) => (
                ...
                >
                  ...
                  {activeLocation === location.name && (
                    <div className="tag tag-primary absolute -top-2 -right-2 text-xs shadow-lg">
                      Active
                    </div>
                  )}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-4 flex-1">
                      <motion.div
                        ...
                        className={`p-3 rounded-xl flex-shrink-0 ${getMarkerColor(location.status)}`}
                      >
                        <MapPin className="h-4 w-4 text-primary" />
                      </motion.div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg font-bold text-primary group-hover:text-solar-accent transition-colors">
                          {location.name}
                        </h3>
                        <p className="text-solar-accent font-semibold text-base">{location.type}</p>
                        <p className="text-tertiary text-sm">Capacity: {location.capacity}</p>
                      </div>
                    </div>
                    <span className={`tag ${getStatusColor(location.status)} text-sm`}>
                      {location.status}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <span className="text-solar-accent text-sm font-medium flex items-center gap-2">
              ...
              {searchQuery && filteredLocations.length === 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  className="text-center py-8 text-tertiary card card-glass"
                >
                  <Search className="h-12 w-12 mx-auto mb-4 text-tertiary/50" />
            ...
            >
              {[
                ...
                { 
                  status: "Construction", 
                  capacity: underConstructionCapacity, 
                  color: "solar-warning",
                  icon: Building2,
                  label: "In Progress"
                },
                ...
                <motion.div
                  ...
                  onClick={resetView}
                >
                  <div className={`bg-${stat.color}/20 w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3`}>
                    <stat.icon className={`h-5 w-5 text-${stat.color}`} />
                  </div>
                  <h4 className={`text-sm font-bold text-${stat.color} mb-1`}>{stat.status}</h4>
                  <p className={`text-2xl font-extrabold text-${stat.color}`}>{stat.capacity}MW</p>
                  <p className={`text-${stat.color}/80 text-xs`}>{stat.label}</p>
                </motion.div>
              ))}
            </motion.div>
            ...
            >
              ...
              <motion.p
                initial={{ opacity: 0, scale: 0.9 }}
                whileInView={{ opacity: 1, scale: 1 }}
                className="text-4xl font-black gradient-text-solar mb-4"
              >
                {totalCapacity}MW
              </motion.p>
              <p className="text-tertiary text-lg mb-4">Combined Solar Capacity</p>
              
              <div className="flex justify-center gap-4 text-sm">
                <span className="text-solar-success font-medium">Operational: {operationalCapacity}MW</span>
                <span className="text-solar-warning font-medium">Construction: {underConstructionCapacity}MW</span>
                <span className="text-solar-accent font-medium">Pipeline: {pipelineCapacity}MW</span>
              </div>
...
export default NationalFootprint