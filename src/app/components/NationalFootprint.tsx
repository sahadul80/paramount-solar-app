...
import { MapPin, Navigation, ZoomIn, ZoomOut, Compass, Search } from 'lucide-react'
...
const NationalFootprint = () => {
  ...
  const locations: Location[] = [
    {
      ...
      capacity: "30MW",
      slug: "/projects/lalmonirhat-project"
    },
    {
      name: "PABNA 1",
    ...
    },
    {
      ...
      capacity: "25MW",
      slug: "/projects/moulvibazar-project2"
    },
    {
      name: "MOULVIBAZAR 1",
      ...
      capacity: "10MW",
      slug: "/projects/moulvibazar-project"
    },
    {
      name: "PABNA 2",
  ...
  const getMarkerColor = (status: string) => {
    switch (status) {
      ...
      case 'Under Construction': return 'bg-yellow-500 hover:bg-yellow-600'
      case 'Pipeline': return 'bg-blue-500 hover:bg-blue-600'
  ...
  // Convert lat/lng to SVG coordinates for Bangladesh map
  const getCoordinates = (lat: number, lng: number) => {
    // Bangladesh bounds approximately
    const minLat = 20.5, maxLat = 26.7
    const minLng = 88.0, maxLng = 92.7
    
    // SVG viewBox coordinates and dimensions
    const svgX = -10, svgY = 0
    const svgWidth = 450, svgHeight = 600
    
    const x = ((lng - minLng) / (maxLng - minLng)) * svgWidth + svgX
    const y = svgHeight - ((lat - minLat) / (maxLat - minLat)) * svgHeight + svgY
  ...
  const operationalCapacity = locations
    ...
    .reduce((sum, loc) => sum + (parseInt(loc.capacity || '', 10) || 0), 0)
  ...
  return (
    <section id="national-footprint" className="bg-white">
      <div className="container mx-auto px-4 py-8">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-800">NATIONAL FOOTPRINT</h2>
        ...
        >
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search projects by name, type, status, or capacity..."
              value={searchQuery}
          ...
          {searchQuery && (
            <p className="text-sm text-gray-600 mt-2">
              Found {filteredLocations.length} project{filteredLocations.length !== 1 ? 's' : ''} matching &ldquo;{searchQuery}&ldquo;
            </p>
          )}
        </motion.div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <motion.div
            ...
            className="flex-1"
          >
            <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-2xl p-4 border-2 border-gray-200 relative overflow-hidden">
              {/* Map Container with Zoom and Drag */}
              <div 
                ref={mapRef}
                ...
                onWheel={handleWheel}
              >
                {/* Bangladesh SVG Map - Add your SVG paths here */}
                <svg
                ...
                {/* Location Markers - Positioned using actual coordinates */}
                {locations.map((location, index) => {
                  const coords = getCoordinates(location.coordinates.lat, location.coordinates.lng)
                  ...
                  return (
                    <motion.button
                      ...
                      className={`absolute w-6 h-6 rounded-full border-2 border-white shadow-lg cursor-pointer z-10 flex items-center justify-center group ${
                        isActive ? 'ring-2 ring-white ring-offset-2' : ''
                      } ${getMarkerColor(location.status)}`}
                      style={{
                        left: `${((coords.x + 10) / 450) * 100}%`,
                        top: `${(coords.y / 600) * 100}%`,
                        transform: 'translate(-50%, -50%)'
                      }}
                      onClick={() => handleMarkerClick(location.slug, location.name)}
                      title={`Click to view ${location.name} project`}
                    >
                      {/* Pulsing animation */}
                      <motion.div
                        className="absolute inset-0 rounded-full border-2"
                        style={{ borderColor: getMarkerColor(location.status).replace('bg-', 'border-').replace(' hover:bg-', '') }}
                        animate={{
                          scale: [1, 2],
                          opacity: [0.6, 0]
                        }}
                      ...
                      {/* Tooltip */}
                      <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none shadow-lg z-20">
                        <div className="font-semibold">{location.name}</div>
                        <div className="text-gray-300">{location.capacity} • {location.status}</div>
                        <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-gray-900 rotate-45"></div>
                      </div>

                      {/* Location label */}
                      <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-xs font-semibold text-gray-700 opacity-70 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                        {location.name}
                      </div>
                    </motion.button>
                  )
                ...
                {/* Map Legend */}
                <div className="absolute bottom-2 left-2 bg-white/95 backdrop-blur-sm p-4 rounded-lg shadow-lg border border-gray-200 z-10">
                ...
                {/* Map Title */}
                <div className="absolute top-2 right-2 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="absolute bottom-4 right-4 flex flex-col gap-2 z-20">
                <button
                  onClick={zoomIn}
                  className="bg-white/90 backdrop-blur-sm p-3 rounded-lg shadow-lg border border-gray-200 hover:bg-white transition-colors"
                  title="Zoom In"
                >
              ...
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-lg border border-gray-200 shadow-sm z-20">
              ...
              {/* Mobile Instructions */}
              <div className="lg:hidden absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/70 text-white text-xs px-3 py-2 rounded-lg backdrop-blur-sm">
          ...
          >
            <div className="max-h-[450px] overflow-y-auto p-2 space-y-4 custom-scrollbar">
              {(searchQuery ? filteredLocations : locations).map((location, index) => (
                <motion.div
                  ...
                  className="bg-gray-50 p-4 rounded-lg hover:shadow-md transition-all border border-gray-200 cursor-pointer group"
                  onClick={() => handleLocationClick(location.slug, location.name)}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      <div className={`p-2 rounded-full flex-shrink-0 ${getMarkerColor(location.status).replace('hover:', 'group-hover:')}`}>
                      ...
                      <div>
                        ...
                        <p className="text-green-600 font-semibold text-sm">{location.type}</p>
                        <p className="text-gray-600 text-sm">Capacity: {location.capacity}</p>
                        <p className="text-gray-500 text-xs">
                          Coordinates: {location.coordinates.lat}°N, {location.coordinates.lng}°E
                        </p>
                      </div>
                    </div>
                    <span className={`inline-flex items-center p-2 rounded-full text-xs font-medium ${getStatusColor(location.status)}`}>
                      {location.status}
                    </span>
                  </div>
                </motion.div>
              ...
              {searchQuery && filteredLocations.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>No projects found matching &ldquo;{searchQuery}&ldquo;</p>
                  <button
                    onClick={clearSearch}
            ...
            >
              <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-4 text-white text-center shadow-lg">
                <h4 className="text-sm font-bold mb-1">Operational</h4>
                <p className="text-2xl font-bold">{operationalCapacity}MW</p>
              ...
              <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-4 text-white text-center shadow-lg">
                <h4 className="text-sm font-bold mb-1">Under Construction</h4>
                <p className="text-2xl font-bold">{underConstructionCapacity}MW</p>
              ...
              <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-4 text-white text-center shadow-lg">
                <h4 className="text-sm font-bold mb-1">Pipeline</h4>
                <p className="text-2xl font-bold">{pipelineCapacity}MW</p>
                <p className="text-blue-100 text-xs">Planned</p>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl p-6 text-white text-center shadow-lg"
            >
              <h4 className="text-lg font-bold mb-2">Total Electric Power</h4>
              <p className="text-3xl font-bold mb-2">
                {operationalCapacity + underConstructionCapacity + pipelineCapacity}MW
              </p>
              <p className="text-gray-300 text-sm">Combined Capacity Across All Projects</p>
            </motion.div>
          </motion.div>
        </div>
      </div>
