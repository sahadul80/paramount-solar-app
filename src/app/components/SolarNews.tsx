...
export default function SolarNews() {
  ...
  const [pageSize, setPageSize] = useState(8); // Changed to 6 for better grid layout
  const [total, setTotal] = useState<number | null>(null);

  const buildUrl = (q = query, c = country, p = page, ps = pageSize) => {
          ...
          {articles.map((article) => (
            ...
            <article className="card card-interactive group h-full flex flex-col overflow-hidden">
              ...
              <div className="flex-1 p-2 flex flex-col">
                <h6 className="text-sm sm:text-md font-bold text-primary line-clamp-3 group-hover:text-solar-primary transition-colors">
                  {article.title}
                </h6>
                
                {article.description && (
                  <p className="text-sm text-secondary line-clamp-3 m-2 flex-1">
                    {article.description}
                  </p>
                )}
                
                <div className="mt-auto p-2 border-t border-primary bg-foreground btn btn-sm w-full justify-center group/link">
                    Read Full Article
        ...
        <div className="flex-center m-2 sm:m-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {totalPages && totalPages > 5 && <span className="text-tertiary">...</span>}
            </div>
